define(['webuploader','requestUtil','layer','app','requestUtil'], 
function(WebUploader,requestUtil,layer,app,requestUtil) {
	var extendPara = {};
	var Export = {};
    // 当domReady的时候开始初始化
...
//            sendAsBinary: true,
            ...
            //server: '/redPacReward/createApplyImage',
            //允许选择的图片格式
            accept: {
              title: 'Images',
        ...
        function setState( val ) {
            ...
            switch ( state ) {
                ...
                case 'finish':
                    stats = uploader.getStats();
                    if (stats.successNum) {
                    	//storageUtil.putParam();
//                        layer.msg("上传成功",{icon:1,time:1000},function(){
//                        	
//                        		//$location.path("/uploadSuccess");
//                        })
                    	
                    	layer.msg('图片上传成功', {icon:1,time: 0 ,btn: ['继续上传', '关闭'],yes: function(index){
                    			location.reload();
                    		    layer.close(index);
                    		  }
                    	});
        ...
        $upload.on('click', function() {
            ...
            if ( state === 'ready' ) {
                ...
                var valideteCode = $(".requestCode").val();
                var campaignId = requestUtil.getUrlParam().campaignId;
	            requestUtil.post("redPacReward/createApply",{"campaignId": campaignId, "telephone":telPhone,"authCode":valideteCode}).then(function(resp){
